...
def extract_waytoagi_item_doc(block_data: dict[str, Any]) -> dict[str, str]:
    text_obj = block_data.get("text", {}) if isinstance(block_data, dict) else {}
    ...
    for value in num_to_attrib.values():
        if not (isinstance(value, list) and len(value) >= 2):
            continue
        if value[0] != "inline-component":
            continue
        raw_component = value[1]
        ...
        try:
            ...
            continue
        if str(component.get("type") or "") != "mention_doc":
            continue
        data = component.get("data", {}) if isinstance(component, dict) else {}
        title = str(data.get("title") or "").strip()
        raw_url = str(data.get("raw_url") or "").strip()
        if title or raw_url:
            return {"title": title, "url": raw_url}
    return {}


def extract_waytoagi_item_url(block_data: dict[str, Any], fallback_url: str) -> str:
    meta = extract_waytoagi_item_doc(block_data)
    raw_url = str(meta.get("url") or "").strip()
    return raw_url or fallback_url


def score_waytoagi_item_url(url: str, root_url: str, history_url: str | None) -> int:
    u = (url or "").strip()
    if not u or u == "#":
        return 0
    if u == root_url or (history_url and u == history_url):
        return 1
    return 2
...
def extract_waytoagi_recent_updates_from_block_map(
    ...
    for bid, block in block_map.items():
        ...
        if not day:
            continue
        detail = clean_update_title(block_text(bd))
        if not detail:
            continue
        key = (day.isoformat(), detail)
        if key in seen:
            continue
        seen.add(key)
        doc_meta = extract_waytoagi_item_doc(bd)
        article_title = str(doc_meta.get("title") or "").strip() or detail
        article_url = str(doc_meta.get("url") or "").strip() or page_url
        updates.append(
            {
                "date": day.isoformat(),
                "title": article_title,
...
def fetch_waytoagi_recent_7d(session: requests.Session, now_utc: datetime, root_url: str) -> dict[str, Any]:
    ...
    for item in updates:
        ...
        if not (key[0] and key[1]):
            continue
        current = dedup_updates.get(key)
        if current is None:
            dedup_updates[key] = item
            continue

        current_score = score_waytoagi_item_url(
            str(current.get("url") or ""),
            root_url,
            history_url,
        )
        new_score = score_waytoagi_item_url(
            str(item.get("url") or ""),
            root_url,
            history_url,
        )
